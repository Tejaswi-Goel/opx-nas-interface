...
def physical_ports_for_front_panel_port(fp_obj):
    ...
    for i in ports:
        try:
            ...
            print("Physical port get failed")
            pass
...
#Get list of interfaces in given port group id
def get_port_list(pg_id):
	pg_list = nas_os_pg_state_list(d={'dell-pg/port-groups-state/port-group-state/id':pg_id})
	list= pg_list[0]['data']['base-pg/dell-pg/port-groups-state/port-group-state/front-panel-port']
	fp_port_list = [ bytearray_utils.from_ba(i, 'uint8_t') for i in list ]
	return fp_port_list
...
def get_port_breakoutCap_currentMode_mode(fp_port):
	fp_utils.init()
	port = fp.find_front_panel_port(fp_port)
